...
// Returns a *NotSingularError when exactly one Transaction entity is not found.
...
func (tq *TransactionQuery) sqlAll(ctx context.Context) ([]*Transaction, error) {
	...
	if query := tq.withReview; query != nil {
		...
		ids := make(map[int32]*Transaction, len(nodes))
		...
		var (
			edgeids []int32
			edges   = make(map[int32][]*Transaction)
		)
		_spec := &sqlgraph.EdgeQuerySpec{
			...
			Predicate: func(s *sql.Selector) {
				s.Where(sql.InValues(transaction.ReviewPrimaryKey[0], fks...))
			},
